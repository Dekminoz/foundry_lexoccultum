...
export class SimpleActorSheet extends ActorSheet {
  ...
  onSearch(event) {
    ...
    if (str.length > 2) {
      $(".skill-label").each(function () {
        if ($($(this).children()[0]).text().indexOf(str) == -1) {
          $(this).parent().hide();
        } else {
          $(this).parent().show();
        }
  ...
  calcultateCp() {
    ...
    let weaponPoints = 0;
    $("input.activateWeapon").each(function () {
      if ($(this).is(":checked")) {
        weaponPoints = $(this).data("pa");
      }
    });
    let armorPoints = 0;
    $("input.activateArmor").each(function () {
      if ($(this).is(":checked")) {
        armorPoints = Number($(this).data("pt")) / 2;
      }
    });
    let move = 10 + Number(
      $('input[name="data.skills.body_control.value"]').val()
    )+Number(
      $('input[name="data.traits.dexterity.value"]').val()
    );
